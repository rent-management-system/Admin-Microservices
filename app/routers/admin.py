...
from app.services.admin import get_users, update_user, get_properties, approve_property, get_health
...
async def update_user_endpoint(user_id: str, data: dict, admin: dict = Depends(get_current_admin)):
    user = await update_user(user_id, data)
    await logger.info("Updated user", user_id=user_id, admin_id=admin["id"])
    return user

@router.get("/properties", response_model=List[PropertyResponse])
async def list_properties(admin: dict = Depends(get_current_admin)):
